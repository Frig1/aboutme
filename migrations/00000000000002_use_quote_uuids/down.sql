...
ALTER TABLE quote_sections RENAME TO quote_sections_uuid;
ALTER TABLE quotes RENAME TO quotes_uuid;
...
CREATE TABLE quote_id_map (
    ...
    integer_id INTEGER NOT NULL UNIQUE
);
...
SELECT map.integer_id, old.user_id, old.title, old.description, old.created_at, old.updated_at
FROM quotes_uuid AS old
JOIN quote_id_map AS map ON map.uuid = old.id;
...
SELECT
    ...
    old.price_cents
FROM quote_sections_uuid AS old
JOIN quote_id_map AS map ON map.uuid = old.quote_id;

DROP TABLE quote_sections_uuid;
DROP TABLE quotes_uuid;
